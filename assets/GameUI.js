 ...
 */
    buildDeck(gameDeck) {
        ...
        gameDeck.forEach((card, cardIndex) => {
            ...
            const iElement = document.createElement('img');
            iElement.setAttribute('src', `/build/images/${card.symbol}`);
            iElement.setAttribute('class', 'imghidden');
            iElement.setAttribute('id', `img${cardIndex}`);
 ...
 */
    setupForNewGame(event) {
        ...
        document.querySelector('.game-board').setAttribute('style', 'display: flex; display: flex; justify-content: center; align-items: center; flex-direction: column;');
